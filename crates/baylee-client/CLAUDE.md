...
- A summoning-sick creature (`board::asleep`, creatures only, CR 302.6) has a slow wave of moonlight run over it (`ShellKind::Wave`; over its own print by the owner's okay of 25.09, never where it would land on another print, `a_wave_never_lands_on_another_cards_print`), and its plate inks moon-grey (`PLATE_NIGHT`), which a fanned row still shows. The strip's moon bit stays empty; never renumber the label.
- Offers (`glow::OFFERS`: activatable, reachable, armed, will-tap) are light on the felt (`floormat.rs`, `floor.wgsl`): a child quad under the card at `FLOOR_RUNG`, never drawn on the card material. The count badge (`badgemat.rs`) stands at the top-right corner, outside the card: `Above` in a duel (upright on a tapped card, `table::keep_upright`, which also keeps the plate upright); `Upright` undoes only the tap, the twist about the face's normal (`table::tap_of`): a flier's bank and pitch carry plate and badge with their card, or the print rises through a plate's inner end (`a_plate_rides_its_card_through_a_fliers_bank`) and `Beside` at a ring, where a merged card holds the gap after it (`HELD_PITCH`). A merged group stands on a pile of slabs jogged by `PILE_JOG` 0.08 (`SLAB_EDGE_COLOR`). What lies under a host peeks a whole `ATTACH_PEEK` or none; where it folds, the host wears the attachment mark in the badge's place (`cardplate::attached_word`, `BADGE_ATTACHED`; shader `tucked_mark`), a ring row holds the cell after a host (`Lane::gaps`), a tapped host keeps them in its untapped footprint, and hover lays them beside the preview under ATTACHED (#305).
- A protected permanent wears a shell (`shellmat.rs`, `shell.wgsl`). Rim, rings and wall are exactly transparent over their own print (a view-ray mask; `every_colour_but_the_domes_and_the_waves_carries_the_mask`); a dome and the wave may lie over their own print (the owner, 25.09, `docs/legal.md` §3). `table::fit_the_shells` stands a shell up or lays it on the felt from the live transforms, and one guard, `shellmat::shell_stands` over a profile that follows the throw's real direction, keeps every shell off other prints. Indestructible is a darksteel rim, a flat lip then a quarter-round bevel, that lies down as a rod. The hover preview wears the same shells, always standing (`shellui.rs`, `shell_ui.wgsl`, seen from straight over the card, lit at `table::DUEL_LEAN`); both shaders draw from `shell_common.wgsl`, and `shellmat::Shells::of(group)` is the one door to what a card wears (keywords and summoning sickness); `Shells::standing` orders the preview's looks (wall, wave, rim), so a new shell is a look there plus an arm in both `shell.wgsl` and `shell_ui.wgsl`. A standing dome whose foot is on the felt, and the wall, cast shadows on the felt (`ShellKind::Shade`, `SHADE_RUNG`): under every face, masked, a dome's faded out before a hover lifts it to a face (`a_shadow_lies_on_the_felt_outside_its_card`).
