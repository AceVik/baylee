...
- `StackText{face,line,of}` indexes the card in `rules` (`RulesFace`; for a copy the copied card, CR 707.2), not the source's. `cardtext::sentence` is the one door (ability sheet, stack, cast chooser): the player's printing (`card_face::sentence_blocks`) if it pairs with the `of` English sentences, else compiled English Oracle, never blank. Use `StackText.face`, not the current face (CR 113.7a; §"Which ability is on the stack").
- `hud::ledge` rebuilds the Actions Bar per sentence (`LedgeRevision`); `drawer` (centre), `tray` strip (right), `pool` (left), `menu` (burger: draw offer, concede, version) keep own revisions and survive it; tray and pool share `strip_node`. Sort buttons at `EDGE`, `TOOL_H`; `arrange` takes `tools_reserved(window_w)`.
