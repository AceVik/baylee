...
// PARTIAL — the +1 animates a land (3/3 Elemental, vigilance/hexproof/haste until your next
// turn), the −2 mills three, and the −7 emblem grants "you may play lands from your
// graveyard". The other clauses have no DSL support; see the NOT SUPPORTED lines below.
...
card!(
    ...
    coverage = Coverage::Partial(
        "no ability-granting static the engine enforces; no effect that puts a card from among \
         the milled cards into its owner's hand; no modifier that lets a player cast permanent \
         spells out of a graveyard",
    ),
    abilities = &[
        // NOT SUPPORTED: "Lands you control have \"{T}: Add one mana of any color.\"" —
        // Modifier::GrantActivated exists, but ability-granting statics are still listed as
        // unenforced, so every land would tap for nothing.
        loyalty!(
            1,
            &[
                Effect::continuous(
