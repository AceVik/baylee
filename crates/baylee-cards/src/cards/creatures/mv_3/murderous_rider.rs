...
// 2 life." The dies trigger and the adventure frame are not written (see the NOT SUPPORTED lines).
...
card!(
    ...
    coverage = Coverage::Partial(
        "Swift End is not marked as an Adventure (FaceDef::adventure), so it resolves to the graveyard; and the dies trigger is unwritten, because PutTargetOnBottomOfLibrary moves its target from whatever zone it is in and the triggering card is not re-checked as a target, so a Rider that left the graveyard in response would still be put on the bottom",
    ),
    ...
    // check that the card is still in the graveyard (CR 400.7).
    // NOT SUPPORTED: "(Then exile this card. You may cast the creature later from exile.)" —
    // sayable (`FaceDef::adventure`, as Brazen Borrower), not yet set on Swift End.
    faces = &[
        face!(
