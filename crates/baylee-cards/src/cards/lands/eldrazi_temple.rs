...
// PARTIAL — {C} always, and {C}{C} restricted to colorless Eldrazi
// spells; the printed "or activate abilities of colorless Eldrazi" has no
// variant, so the stricter of the two arms is the one written.
...
card!(
    ...
    abilities = &[
        ...
        // restriction is written for the spell half; a `ManaRestriction` is
        // a `Filter` over the spell being cast, and nothing in the DSL
        // points one at an activated ability.
