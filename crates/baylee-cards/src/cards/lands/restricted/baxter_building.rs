...
// PARTIAL — {C} and {4}, {T} four mana in any combination are built; toughness 4+ activation condition cannot be filtered.
...
card!(
    ...
    coverage = Coverage::Partial(
        "Filter has no ToughnessAtLeast variant, so activating only if you control a creature with toughness 4 or greater cannot be checked"
    ),
    abilities = &[
        ...
        // NOT SUPPORTED: "{4}, {T}: Draw a card. Activate only if you control a creature with toughness 4 or greater."
    ],
);
