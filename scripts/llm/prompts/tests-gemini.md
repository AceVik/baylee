...
18. **Einsatzschwäche (CR 302.6).** Eine Kreatur, die in diesem Zug ins Spiel
...
19. **`.battlefield(seat, …)` und `.hand(seat, …)` ersetzen.** Ein zweiter
...
22. **„… unless you sacrifice/return …" fragt kein Ja/Nein.**
...
23. **Eine gedruckte Mana-Fähigkeit** (Sol Ring, ein Nichtgrundland, eine
...
24. **`cast_with_floating` wirkt, verrechnet aber nicht.** Erst
   `pass_until(&mut engine, stack_is_empty)` lässt den Zauber auflösen —
   vorher ist nichts auf dem Schlachtfeld.
25. **Eine ausgelöste Fähigkeit eines Permanents wirkt nur auf dem
...
26. **Der Gegner zieht auch.** In seinem Ziehschritt verliert seine
