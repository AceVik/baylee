...
- The front door's music is our own score, synthesised in code (`client-core::music`, `docs/legal.md` §5; never a scene module, sample or borrowed tune). It streams on the audio thread (never pre-rendered or run in a frame) and plays only on `Screen::SignIn` while `DuelPhase::Closed`. A faded-out player is despawned. `ClientSettings.music` is per device, and a mute sits on the faces themselves (WCAG 1.4.2).
