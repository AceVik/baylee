...
say who owns a file — `// GENERATED STUB` (nobody has finished it) and
`// IMPLEMENTED — generated by xtask codegen` (a reader wrote it in full) —
and `codegen` rewrites every file carrying either, on every run. A card with
...
currently 16) is asserted in gamehost and client tests — bump it on any breaking
