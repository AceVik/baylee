...
- Gateway: `PORT`, `DATABASE_URL` (required), `BAYLEE_DB_POOL` (8), `STORE_PATH` (import-only, not gitignored), `BAYLEE_ART_PATH`/`BAYLEE_DECK_IMAGE_PATH` (`off` disables; empty also refuses uploads), `BAYLEE_REGISTRATION=off`, `BAYLEE_TRUSTED_PROXIES`, `BAYLEE_AGENT_TOKEN` (none = no agent), `BAYLEE_SMTP_URL`/`BAYLEE_MAIL_FROM`/`BAYLEE_PUBLIC_URL` (no SMTP = no mail or confirmation), `BAYLEE_ENGINE_URL` (set for a remote agent).
